// g++ -O3 allocator_bench_all.cpp pmalloc_intrusive.cpp pmalloc_binary_heap.cpp pmalloc_pairing_heap.cpp -o benchmark
#include <cstddef>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
static void matmul_naive_raw(const T* A, const T* B, T* C,
...
static void print_alignment(const char* label, void* p) { // STILL TESTING I DO NOT FULLY UNDERSTAND
    if (p != nullptr) {
...
static double max_abs_diff_vec(const std::vector<T>& a, const std::vector<T>& b) {
...
static void run_raw_compare_all(const std::vector<AllocAPI>& apis,
    ...
    std::vector<std::vector<T>> C_all; // each result vector C per API indexed by apii
    ...
    while (apii < apis.size()) {
        ...
        std::printf("alloc: %.3fs | copy-in: %.3fs | gemm(best of %d): %.3fs | copy-out: %.3fs | free: %.3fs\n",
...
static void run_pmatrix_compare_all(const std::vector<AllocAPI>& apis,
    ...
    std::printf("Dims: M=%zu, K=%zu, N=%zu | Pool=%.1f MiB | BS=%zu\n",
                M, K, N, POOL_BYTES / (1024.0*1024.0), blockSize);
    ...
    while (idx < apis.size()) {
        ...
        if (api.init_pool_with_bytes != nullptr) api.init_pool_with_bytes(POOL_BYTES);
        bind_matrix_to(api);

        // Build A,B with fixed seeds
        PMatrix A(M,K); A.fill_random(111);
        PMatrix B(K,N); B.fill_random(222);
        ...
        if (same_local == false) {
            ...
            std::printf("[MISMATCH] blocked vs naive within %s (max |Delta| = %.3e)\n", api.name, maxd);
        } else {
...
int main() {
    ...
    AllocAPI intrusive_api {
        "intrusive (pool)",
        pmem_intrusive::init_pool_with_bytes,
        pmem_intrusive::destroy_pool,
        pmem_intrusive::pmalloc,
        pmem_intrusive::pfree,
        pmem_intrusive::pmemcpy
    };
    AllocAPI binheap_api {
        "binary-heap (pool)",
        pmem_binheap::init_pool_with_bytes,
        pmem_binheap::destroy_pool,
        pmem_binheap::pmalloc,
        pmem_binheap::pfree,
        pmem_binheap::pmemcpy
    };
    AllocAPI pairing_api {
        "pairing-heap (pool)",
        pmem_pairing::init_pool_with_bytes,
        pmem_pairing::destroy_pool,
        pmem_pairing::pmalloc,
        pmem_pairing::pfree,
        pmem_pairing::pmemcpy
    };
    AllocAPI libc_api {
        "malloc/free (libc)",
    ...
    std::printf("Dims: M=%zu, K=%zu, N=%zu | Pool=%zu MiB | Repeats=%d | Block=%zu\n", M, K, N, POOL_BYTES / MiB, REPEATS, PMAT_BS);

    // time alloc/copies/GEMM per allocator
    // only does one big allocation and free per allocator
    // the bottleneck is that for some reason when allocated with our allocator the actual computation is slower than with libc malloc
