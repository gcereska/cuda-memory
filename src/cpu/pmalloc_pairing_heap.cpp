...
struct BlockHeader {
  ...
  BlockHeader* prev;  // used-list prev (not used by heap itself) // only used
                      // for used list
  BlockHeader* next;  // used-list next (not used by heap itself) // only used
                      // for used list
};
...
inline BlockFooter* footer_from_head(BlockHeader* h) {
  return reinterpret_cast<BlockFooter*>(reinterpret_cast<std::byte*>(h) +
                                        HEADER_SIZE + h->size);
}
inline void* user_ptr(BlockHeader* h) {
  return reinterpret_cast<void*>(reinterpret_cast<std::byte*>(h) + HEADER_SIZE);
}
inline BlockHeader* header_from_user(void* p) {
  return reinterpret_cast<BlockHeader*>(reinterpret_cast<std::byte*>(p) -
                                        HEADER_SIZE);
}
...
inline BlockHeader* right_neighbors_header(BlockHeader* h) {
  std::byte* after =
      reinterpret_cast<std::byte*>(footer_from_head(h)) + FOOTER_SIZE;
...
inline void ph_clear() {
  // We can walk and delete every node, but since nodes are freed only when
  // blocks leave the heap, and were destroying the pool here, we can just drop
  // pointers and let the OS reclaim. or i will iterative-delete roots:
...
inline BlockHeader* free_list_top() { return ph_top(); }

inline BlockHeader* free_list_pop_max() { return ph_pop_max(); }
...
void init_pool_with_bytes(std::size_t bytes) {
  ...
  if (g_pool == nullptr) {
    std::fprintf(stderr,
                 "init_pool_with_bytes: pool alloc failed for %zu bytes\n",
                 bytes);
    std::abort();
  }
  g_pool_size = bytes;
