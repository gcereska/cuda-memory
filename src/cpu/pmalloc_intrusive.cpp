...
inline BlockHeader* right_neighbors_header(BlockHeader* h) {
  ...
  if (after + HEADER_SIZE > g_pool + g_pool_size)
    return nullptr;  // not enough room for a header
...
inline BlockHeader* left_neighbors_header(BlockHeader* h) {
  ...
  if (footer_from_head(left_header) != left_footer)
    return nullptr;  // footer must match its header
  return left_header;
}
...
inline BlockHeader* coalesce(BlockHeader* h) {
  ...
  while (true) {
    ...
    if (L != nullptr && L->free) {
      ...
      const std::size_t new_size =
          (HEADER_SIZE + L->size + FOOTER_SIZE + h->size);
      L->size = new_size;
  ...
  while (true) {
    ...
    if (R != nullptr && R->free) {
      free_list_remove(R);
      const std::size_t new_size =
          (HEADER_SIZE + h->size + FOOTER_SIZE + R->size);
      h->size = new_size;
      footer_from_head(h)->header = h;
    } else {
      break;
