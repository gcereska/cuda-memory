...
// these are all relatively simple helper functions to make the pointer math
// easier
int16_t get_offset(char* from, char* to) { return (char*)to - (char*)from; }

BlockFooter* get_footer(BlockHeader* header) {
  return (BlockFooter*)((char*)header + get_node_size(header) +
                        sizeof(BlockHeader));
}
...
BlockHeader* get_prev_header(BlockHeader* currentHeader) {
  ...
  BlockFooter* prev_footer =
      (BlockFooter*)((char*)currentHeader - sizeof(BlockFooter));
  BlockHeader* prevHeader =
      (BlockHeader*)((char*)prev_footer + prev_footer->headerOffset);
  return prevHeader;
}
...
BlockHeader* get_next_header(BlockHeader* currentHeader) {
  ...
  if ((currentHeaderOffset + temp_size + sizeof(BlockHeader) +
       sizeof(BlockFooter)) >= MEM_POOL_SIZE) {
    return NULL;
  }

  BlockHeader* nextHeader =
      (BlockHeader*)((char*)currentHeader + temp_size + sizeof(BlockFooter) +
                     sizeof(BlockHeader));
  return nextHeader;
}
...
void list_remove(BlockHeader** list, BlockHeader* deletionNode) {
  ...
  if (deletionNode->nextOffset != 0) {
    currentNodeNext =
        (BlockHeader*)((char*)deletionNode + deletionNode->nextOffset);
  }

  if (deletionNode->prevOffset != 0) {
    currentNode_prev =
        (BlockHeader*)((char*)deletionNode + deletionNode->prevOffset);
  }

  // Fix the previous node's next pointer
  if (currentNode_prev != NULL) {
    if (currentNodeNext != NULL) {
      currentNode_prev->nextOffset =
          get_offset((char*)currentNode_prev, (char*)currentNodeNext);
    } else {
  ...
  if (currentNodeNext != NULL) {
    if (currentNode_prev != NULL) {
      currentNodeNext->prevOffset =
          get_offset((char*)currentNodeNext, (char*)currentNode_prev);
    } else {
...
void init_cpu_buffer() {
  ...
  header->fullSize =
      (MEM_POOL_SIZE - (sizeof(BlockHeader) + sizeof(BlockFooter))) & 0x7FFF;
  header->nextOffset = 0;
  header->prevOffset = 0;
...
void free_buffer() { free(memBuffer); }

void* cmalloc(int16_t size) {
  if (freeList == NULL ||
      get_node_size(freeList) < (size + (int16_t)sizeof(BlockHeader) +
                                 (int16_t)sizeof(BlockFooter))) {
  ...
  newlyAllocatedFooter->headerOffset =
      get_offset((char*)newlyAllocatedFooter, (char*)newlyAllocatedHeader);
  ...
  uint16_t remainingSize =
      preAllocationSize -
      (sizeToAlloc + sizeof(BlockHeader) + sizeof(BlockFooter));

  if (remainingSize > 0) {
    BlockHeader* newFreeHeader =
        (BlockHeader*)((char*)(newlyAllocatedFooter) + sizeof(BlockFooter));
    newFreeHeader->fullSize =
        remainingSize & 0x7FFF;  // both sets size and sets empty
    newFreeHeader->nextOffset = 0;
    newFreeHeader->prevOffset = 0;

    BlockFooter* newFreeFooter = get_footer(newFreeHeader);
    newFreeFooter->headerOffset =
        get_offset((char*)newFreeFooter, (char*)newFreeHeader);
...
void cfree(void* addressForDeletion) {
  ...
  BlockHeader* deletion_target =
      (BlockHeader*)((char*)addressForDeletion - sizeof(BlockHeader));
  ...
  if (forward_coalesce_valid) {
    ...
    tempSize +=
        sizeof(BlockFooter) + sizeof(BlockHeader) + get_node_size(nextHeader);

    deletion_target->fullSize = tempSize & 0x7FFF;

    BlockFooter* next_block_footer = get_footer(deletion_target);
    next_block_footer->headerOffset =
        get_offset((char*)next_block_footer, (char*)deletion_target);
  ...
  if (backward_coalesce_valid) {
    ...
    tempSize += sizeof(BlockFooter) + sizeof(BlockHeader) +
                get_node_size(deletion_target);
    prevHeader->fullSize = tempSize & 0x7FFF;

    BlockFooter* current_block_footer = get_footer(prevHeader);
    current_block_footer->headerOffset =
        get_offset((char*)current_block_footer, (char*)prevHeader);
