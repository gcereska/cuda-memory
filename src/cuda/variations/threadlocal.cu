...
__device__ __forceinline__ int threads_per_block() {
    return int(blockDim.x) * int(blockDim.y) * int(blockDim.z);
}
...
__device__ void pool_init(std::size_t total_bytes) {
    ...
    if (total_bytes > manager_struct_size) {
        ...
        bytes_per_thread = remaining_bytes / threads_per_block();
        bytes_per_thread = bytes_per_thread & ~(ALIGNMENT - 1);
        // rounds down each pool size to neareast multiple of 8when
...
__device__ void* pmalloc_best_fit(std::size_t size) {
    ...
    if (get_pool_id() >= MAX_THREADS) return nullptr; // throw error in future
...
__device__ void pfree(void* user_ptr) {
    ...
    if (get_pool_id() >= MAX_THREADS) return; // throw an error in future
