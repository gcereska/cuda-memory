...
public:
    ...
    PMatrix(std::size_t rows, std::size_t cols)
    ...
    static PMatrix multiply_naive(const PMatrix& A, const PMatrix& B) {
        if (A.cols() != B.rows()) {
            std::fprintf(stderr, "[PMatrix] multiply_naive: shape mismatch (%zu×%zu) * (%zu×%zu)\n",
        ...
        const T* a=A.data(); const T* b=B.data(); T* c=C.data();
    ...
    // Blocked multiply
    static PMatrix multiply_blocked(const PMatrix& A, const PMatrix& B, std::size_t BS=128) {
        if (A.cols() != B.rows()) {
            std::fprintf(stderr, "[PMatrix] multiply_blocked: shape mismatch (%zu×%zu) * (%zu×%zu)\n",
                         A.rows(), A.cols(), B.rows(), B.cols());
            std::abort();
        }
        PMatrix C(A.rows(), B.cols());
        C.fill_zero();
        const std::size_t M=A.rows(), K=A.cols(), N=B.cols();
        T* c=C.data(); const T* a=A.data(); const T* b=B.data();
...
inline void choose_dims_from_budget(std::size_t target_bytes,
    ...
    M = x;
    N = x;
    K = std::max<std::size_t>(2, static_cast<std::size_t>(x / (aspect <= 0 ? 1.0 : aspect)));
}