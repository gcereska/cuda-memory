...
// this is the most efficient means I could think of to store the meta data
// required, reducing the size from roughly 32 bytes down to just 8 bytes,
// assuming padding

// if the color were packed into the size, it would be possible to make a RBtree
// as well with 8 byte nodes. it only takes 12 bits to represent the entire
// space (in the size variable), so you could use the upper 1 for color and the
// lower 12 for size...

/*
For starters I will just implement a linked list implementation, there is going
to be a lot of weird math to deal with with just a simple linked list so I am
going to get the first implementation down. If the linked list goes well and
proves valuable I will move on to either the AVL or RBTree, RB would be quite
tough I believe, we would need to write it from scratch nearly to account for
all the weird pointer operations and bit mapping
